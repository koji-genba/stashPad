...
- 実行時の挙動に影響し得る変更(状態管理・ルーティング・ビルド系・依存メジャー更新)では、テスト緑に加えて `npm run dev` を起動し Playwright(Chromium は導入済み)で主要画面(`/`・作品詳細・`/history`・`/settings`)を開き、**コンソールエラー・警告が出ないこと**を確認する。テストは全経路を再現しない。backend 未起動時の `/api` への 500 は想定内なので追いかけない
...
- roadmap.md に対応チェックボックスがあれば完了に更新する。ただし **issue の一部だけを完了した場合はチェックを付けず**、完了報告で進捗として明記する
