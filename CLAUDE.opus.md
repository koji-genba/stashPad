...
## セッションの運用(「roadmap を見ながら進めて」のような大まかな指示で自走する場合)

### タスク選定

- open issue と docs/roadmap.md の未完了項目を突合し、roadmap の優先度(信頼性 → 体験 → 基盤)と並び順に従って着手する
- アイデア置き場の項目に着手する場合は、先に issue を起票し(既存 issue の書式: 背景 / 要件案・提案 / 実装メモ / 参考)、roadmap のマイルストーンへ移してから始める
- 1 セッションで欲張らない。**1〜2 issue を完了させて push するまでを 1 サイクル**とし、区切りごとに成果を報告する

### サブエージェントへの委譲(トークン節約)

- メインセッションは設計・レビュー・監査に専念し、実装と調査はサブエージェントに委譲する
- モデルの使い分け: **Haiku** = 調査・docs/コードの要約 / **Sonnet** = 定型的・機械的な実装(依存バンプ、パターンが確立した小修正)/ **Opus** = 設計判断を含む実装・新機能・原因不明のデバッグ
- 委譲プロンプトに必ず含める: 作業ブランチ名と **push 禁止** / issue 本文または要件の全文 / 本ファイルを読んで従う指示 / 完了報告の形式(変更要約・実行した検証コマンドと結果・判断に迷った点)
- 委譲は**結果が同期的に返る形を既定**とし、並列化は互いに独立なタスクに限る。完了通知には依存せず、成果は作業ツリー・コミットの実物で確認する。待ちが空振りする・同期委譲が機能しない環境では、無理に待たず自分の実装に切り替え、その判断を報告に残す

### 監査(サブエージェントの報告を鵜呑みにしない)

- 委譲する**前に**、自分の合格基準(何ができていれば良しとするか)を書き出しておく。報告を読んでから基準を作らない
- 報告された検証結果は最低 1 系統(テストまたは typecheck)を**自分で再実行**して突合する
- diff を自分で読む: コミット構成、テストが仕様を記述しているか、docs(roadmap / design.md §12)更新の有無
- 不合格なら差し戻すか自分で直す。合格したものだけを成果ブランチに統合する

### 止まるべきタイミング

- design.md と矛盾する変更・DB スキーマ変更・破壊的操作(force push、データ削除)が必要になったら、進めずにユーザーに確認する
- 同じ失敗で 3 回詰まったら、状況と試したことを整理して報告し、指示を待つ
