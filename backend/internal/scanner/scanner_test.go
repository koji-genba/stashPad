...
func TestScanTitleExtraction(t *testing.T) {
	...
	testDirs := []struct {
		...
		// issue #65: RJ 番号の直後だけを見る方式のリグレッションテスト。
		// 旧実装(firstUnderscoreAfterRJ)はフォルダ名全体で最初の "_" を探すため、
		// RJ 番号直後が "_" 以外の区切り文字だと、後方の "_" で誤って区切ってしまっていた。
		{"RJ123457_作品名", "作品名"},           // RJ 直後が "_" → 単純ケース
		{"RJ123458-作品名_ver2", "作品名_ver2"}, // RJ 直後が "-" → 旧実装は "ver2" に誤抽出していた
		{"RJ123459 作品名_後編", "作品名_後編"},     // RJ 直後が半角スペース → 旧実装は "後編" に誤抽出していた

		// 区切り文字なし(RJ 番号の直後にいきなりタイトルが続く)場合、TrimLeft は
		// 何も削らないため rest がそのままタイトルになる仕様とする。
		{"RJ123460作品名", "作品名"},
	}
