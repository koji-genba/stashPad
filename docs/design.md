...
```mermaid
flowchart LR
    mini["ミニモード(画面下部固定)<br>サムネ / タイトル / 再生・停止<br>±10秒 / シークバー(細)"]
    full["フルスクリーンモード<br>大シークバー / ±5・±10・±30秒<br>前後トラック / 再生速度 / キュー一覧"]
    mini -- "タップ / スワイプアップ" --> full
    full -- "スワイプダウン / 閉じる" --> mini
```
...
- [x] オーディオプレイヤー フルスクリーンモード(±5/±30秒・キュー一覧)
- [ ] 再生キュー操作(追い足し・並び替え・削除)→ 設計ノート: [queue-design-notes.md](queue-design-notes.md)
