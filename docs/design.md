...
| GET | `/circles?q=` | サークル一覧(作品数付き、ファセット表示用。作品の絞り込みは `/works?circle=` の完全一致を使う) |
...
> 未完了項目の着手順と GitHub issue との対応は [roadmap.md](roadmap.md) で運用する。本節はフェーズの全体像を示す。
...
- [ ] k8s マニフェスト整備
- [ ] お気に入り・ソート強化(issue #72)
- [ ] 画像ビューアの強化(ページジャンプ・プリロード調整、issue #28)
- [ ] PWA 対応(standalone 起動、issue #73)

### Phase 3 — 必要になったら

- [ ] 認証(セッション Cookie ミドルウェア)
- [ ] 再生位置の記憶(続きから再生、issue #71 で Phase 2 相当に前倒し)
- [ ] 評価機能(独自タグ運用で不足したら)
- [ ] FTS5 による全文検索
- [ ] フォルダ監視による自動スキャン
...
| D26 | スリープタイマー(2026-07-12) | playerStore に `sleepMode('off'/'duration'/'endOfTrack')` と `sleepEndsAt` を追加(issue #105)。「N 分後」は期限を**絶対時刻(epoch ms)で保持**し、AudioPlayer が毎秒 `tickSleepTimer` を呼んで `Date.now()` と比較する(残り秒カウントダウンにしないのはタブ非アクティブ/スロットリングでずれるため)。「トラック終端」は `handleEnded` の自動前進を抑止して停止。停止は **pause**(キュー・index・再生位置を保持)で、mid-track pause 時の `flushProgress` により #71 の続きから再開と自動的に整合する。タイマー状態は queue と同じく永続化しない(揮発)。UI はフルスクリーンプレイヤー下部に配置し、作動中は残り時間/終端インジケータと解除ボタンを出す |
