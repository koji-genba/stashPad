...
本ノートの設計を基に実装が完了した。以下は本ノートからの主な差分。

- **uid 方式に変更**: `QueueTrack` に `uid` フィールドを追加し store の `nextUid` で採番。重複追加・ドラッグ並び替えに耐える React key / ドラッグ識別子として使用。設計ノートの複合キー案(`` `${workId}:${path}:${i}` ``)からの変更
- **ドラッグ並び替えに変更**: 「行ごとの ▲▼ ボタン」案ではなくドラッグ方式を採用(オーナー指定)。Pointer Events 自前実装、外部依存追加なし
- **3 アクションに分割**: `enqueueTrack` 1 つではなく用途別の `playTrackNext`(現在トラック直後に挿入)・`replaceQueueWith`(1 曲にして即再生)・`appendToQueue`(末尾追加)の 3 アクション
- **`expanded` を store から廃止し history 駆動に変更**: フルスクリーンプレイヤーとキュー画面の表示状態を zustand ではなく `history`(location.state)で管理する `usePlayerOverlay` フックを採用。`removeFromQueue` の全クリア時に `expanded=false` を設定する必要がなくなり、代わりにキューが空になったら積んだ history エントリを unwind で巻き戻す
- **＋ ボタンではなく ⋮ ボトムシートメニュー**: FileBrowser の audio 行エントリポイントを ＋ ボタンから ⋮ メニュー(ボトムシート)に変更し、上記 3 アクションをまとめて提供

上記以外のセマンティクス(`moveInQueue` の index 追従ロジック・`removeFromQueue` の次トラック送り等)は設計ノートの記述通り実装されている。本ノートは史料として残す。
