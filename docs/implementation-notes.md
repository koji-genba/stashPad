...
```json
{"created": 0, "updated": 1480, "linked": 5, "skipped": 12, "errors": []}
```
...
- サムネイル: 作品ルートから深さ 2 までを探索し、`表紙|cover|jacket|サムネ|main`(大文字小文字無視)を名前に含む画像を優先、なければ自然順で最初の画像。AppleDouble(`._*`)や `.DS_Store` を含む dotfile / dotdir は候補外。優先候補のデコードに失敗した場合は同一 work 内の次候補を試す。長辺 512px に縮小し jpeg (quality 85) で `{data}/thumbs/{work_id}.jpg` へ保存。生成済みならスキップ

## 10. CSV インポータ

- ヘッダ行必須。列は docs/samples/works.csv の 17 列を想定するが、**列順はヘッダ名で解決**する(将来の列追加に耐える)
- 文字コードは UTF-8(BOM 付き許容。先頭の BOM を剥がす)
- `rj_number` で既存 works 行を更新する。CSV はスキャンで見つかったフォルダにメタデータを付与する位置付けなので、`rj_number` に一致する works 行が存在しない行は新規作成せずスキップし、`Result.Skipped` / API レスポンスの `skipped` に件数を返す(issue #76)
- 既存の `root_path IS NULL` 行(過去の CSV インポートで作られた幽霊行、またはスキャン後にフォルダが消えて NULL 化された行)は「既存 works 行」として CSV メタデータ・タグ更新の対象にする。スキャナは後から同じ RJ 番号のフォルダを検出したとき、その行の `root_path` を再リンクできるため、既存行をスキップ対象にはしない(issue #76)
- 手動編集されていない works のカラムは CSV 値で上書きする。`manually_edited=1` の作品は title/circle を CSV 再インポートで上書きしない(issue #64)
